    ...
    public function execshareAction(){
        ...
        $notificationData['time'] = time();
           if(!empty($params["friends"])&& !empty($friends)){
            $me = array("installed"=>1, "id"=>$_SESSION['user_id'], "me"=>1, "name"=>$_SESSION['user_name']);
            array_push($friends, $me);
            foreach($friends as $friend){
                if( in_array($friend['name'], $params["friends"]) || (!empty($friend['me'])) ){
                    if(!empty($friend['installed']) && $friend['installed'] == true){
                        $fr_arr = (!empty($friend['me']))?array('id_tbl_user'=>$friend['id']):$this->qM->getUserById($friend['id']);
                        ...
                        $notificationData['type'] = $return['notif_type'];
        ...
        //$this->_helper->json(array('link' =>'http://'.$this->base_path.'/default/fblinks/'.$type.'?id='.$link_id.'&fb_page=1', 'js_ar_fr' => json_encode($js_ar_fr)));
        $this->_helper->json(array('link' =>'http://'.$this->base_path.'/default/recommendation/index?link_id='.$link_id , 'js_ar_fr' => json_encode($js_ar_fr)));
    }

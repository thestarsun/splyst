...
//    }
    ...
    public function saveLink($data){
        if(!empty($data['comment'])){
            ...
            //$link = array('url'=> $data['url'], 'title'=>$data['title'], 'thumbnail'=> $data['img'] , 'comments' =>json_encode($comments));
            $this->db->query("INSERT INTO links (url, title, thumbnail, comments) VALUES ('".$data['url']."', '".$data['title']."', '".$data['img']."', '".json_encode($comments)."')");
        }else{
            //$link = array('url'=> $data['url'], 'title'=>$data['title'], 'thumbnail'=> $data['img']);
            $this->db->query('INSERT INTO links (url, title, thumbnail) VALUES ("'.$data['url'].'", "'.$data['title'].'", "'.$data['img'].'")');
        }
        ...
        return array("id"=>$data['id'], "exp_thumbnail"=>$data['img'], "exp_title"=>$data['title'], "lastInsertLinkId"=>$link_id);
    }
    ...
    public function rec2link($data){
        ...
        if(!empty($link['id'])){
            ...
            //$this->db->query("UPDATE links SET thumbnail='".$data['thumbnail']."' WHERE id='363'");
            $link_id = $link['id'];
        }else{
            $comments[] = json_decode($data['comments'], true);
            $data['comments'] = json_encode($comments);            
            //$this->db->insert('links', $data);
            $this->db->query("INSERT INTO links (url, title, thumbnail, description, old_rec, comments)VALUES ('".$data['url']."', '".$data['title']."', '".$data['thumbnail']."', '".$data['description']."', '".$data['old_rec']."', '".$data['comments']."')");
            $link_id = $this->db->lastInsertId();
        }
        return $link_id;
    }

...
class App_Google_Bildcontent {
    ...
    public function bild($params){
        ...
        if(!$cache->test('getLinkTag_'.$params['id'])){
            $tag_ids = $this->lM->getLinkTag($params['id']);
            $cache->save($tag_ids, 'getLinkTag_'.$params['id'], array(), 172800);
        }else{
            $tag_ids = $cache->load('getLinkTag_'.$params['id']);
        }
        $mixed_link_data = array();
        if(!empty($tag_ids)){
            ...
            foreach ($tag_ids as $tag_id){
                if (!$cache->test('link_'.$params['type'].'_'.$params['amount'].'_'.$tag_id)){
                    ...
                    if(strtotime('2 days ago') > strtotime($link['date'])){
                        ...
                        $link_data = json_decode($link['data'], true);
                ...
                if(!empty($link_data))
                $mixed_link_data = (!empty($mixed_link_data))?array_merge($link_data, $mixed_link_data):$link_data;
            }
        }
        return $mixed_link_data;
    }
...
//                        $tag = (!empty($tag_tmp)) ? $tag_tmp : str_replace(array('@', '#', '"', '`', "'"), '', $like);
                        ...
                        if (!empty($db_tag_id)){
                            $users_ids = $this->rM->getUserByRecommendation($db_tag_id);
                            if(!empty($users_ids)){
                                foreach ($users_ids as $userID){
                                    $this->addRec2User($db_tag_id, $userID['user_id']);
                                }
                            }
